...
public class ValkeyCartStore : ICartStore
{
    ...
    public ValkeyCartStore(ILogger<ValkeyCartStore> logger, string valkeyAddress)
    {
        ...
        var valkeySslEnv = Environment.GetEnvironmentVariable("VALKEY_SSL");
        bool useSsl = string.Equals(valkeySslEnv, "true", StringComparison.OrdinalIgnoreCase);
        _connectionString = $"{valkeyAddress},ssl={useSsl.ToString().ToLowerInvariant()},allowAdmin=true,abortConnect=false";
